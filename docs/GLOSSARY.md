...
| **Deduplication (declarative)** | `transformation.deduplication` — business key + tiebreaker column (`order_by`) + `keep: latest | earliest`. Compiled directly to a window-function dedup; no agent involved. See [ADR-0010](adr/0010-declarative-deduplication.md). |
| **Quarantine** | `transformation.quarantine` — named row-level validity checks; failing rows are excluded from the curated output and written to a sink table with a captured reason. Independent of the ODCS `quality` block. See [ADR-0011](adr/0011-quarantine-sink.md). |
| **Intent** | The free-text field in `transformation.intent` describing logic that still requires judgment — which rows qualify, which join keys, which source wins on conflict. Guidance for the builder agent, never enforced truth. |
